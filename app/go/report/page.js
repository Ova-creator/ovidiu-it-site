...
export const metadata = {
  ...
  robots: { index: false, follow: false }, // nu indexăm
};
...
export default function GoReportPage() {
  ...
  const totalsAll = useMemo(() => aggregate(rows).sort((a,b)=>b.count-a.count), [rows]);

  const now = Date.now();
  const rows7d = rows.filter(r => now - r.ts <= 7*24*60*60*1000);
  const totals7d = useMemo(() => aggregate(rows7d).sort((a,b)=>b.count-a.count), [rows7d]);

  function downloadCSV() {
    const header = "slug,timestamp,datetime\n";
    const lines = rows.map(r => `${r.slug},${r.ts},"${formatDate(r.ts)}"`).join("\n");
    const blob = new Blob([header + lines], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "tools_clicks.csv"; a.click();
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button className="btn-ghost" onClick={clearData}>Reset local data</button>
      </div>
      ...
      <section className="mb-10">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          {!totals7d.length && <p className="text-sm text-zinc-400">No data yet.</p>}
        </div>
      </section>

      <section>
        <h2 className="text-xl font-semibold mb-3">All-time clicks (this device)</h2>
